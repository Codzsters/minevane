...
if (typeof jQuery.fn.draggable === "undefined") {
    ...
    (function($) {
        ...
        $.fn.draggable = function() {
            ...
            ele.style.top = "88px";
            Drag.init(ele, null, 26, 435, 86, 86);
            return this;
        };
...
var gradX = function(id, _options) {
    ...
    gradx = {
        ...
        apply_style: function(ele, value) {
            ...
            this.update_canvas(value);

        },

        update_canvas: function(value) {
            ...
            if (values.length > 1) {
                values.shift(); // Remove left/right direction option
            }

            var len = values.length, i, colorStops = [],
                gradient = ctx.createLinearGradient(0, 0, canvasWidth, 0);

            var saveData = [];
            for (i = 0; i < len; i++) {
                // If no percentage (position zero), then add 0%
                if (values[i].lastIndexOf("%") == -1) {
                    values[i] += " 0%";
                }

                // Get stop position
                var n = values[i].lastIndexOf(" ");
                var percent = values[i].slice(n).replace('%','');
                var offset = percent / 100;
                var color = values[i].substr(0,n);

                gradient.addColorStop(offset, color);

                saveData[i] = {
                    color: color, position: percent
                }
            }
            ...
            saveCookie('gradient-data', saveData);
        },
        ...
        load_info: function(ele) {
            ...
            if (this.slider_ids.indexOf(ele.id) > -1) { //javascript does not has # in its id
                ...
                if (parseInt(left) > 25 && parseInt(left) < 438) {
        ...
        set_colorpicker: function(clr) {
            gradx.cp.spectrum({
                ...
                palette: ['ffffff','aaaaaa','555555','000000','ffff55',
                'ffaa00','ff5555','aa0000','55ff55','00aa00',
                '55ffff','00aaaa','5555ff','0000aa','ff55ff','aa00aa']
        ...
        load_gradx: function(id, sliders) {
            ...
            //.hide();
            //this.info.hide();
            this.container_width = 410 //HARDCODE;
...
//            if(!gradx.jQ_present){
            ...
            gradx.gx('#gradx_reset').click(function() {
                gradX("#gradX",{
                    sliders: defaultColors
                });
            });
